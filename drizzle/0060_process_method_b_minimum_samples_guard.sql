...
--
-- Semantics mirror the app's baseline counter (`getMethodBEligibilityByProcess` /
-- `countEligibleSamplesByProcess` with `as_of = method_b_unlocked_at`):
-- credit-batch-linked Samples whose credit batch belongs to this process. In-
-- process samples (null `credit_batch_id`) are internal-only (ADR 0016) and the
-- join drops them. A feedstock/condition change opens a NEW process row whose
-- baseline restarts from zero, so prior processes' samples never leak in.
--
-- The count is BOUNDED to the pre-unlock baseline: `sampling_time <
-- method_b_unlocked_at`. The Method-A baseline is the ≥30 samples accumulated
-- BEFORE the switch (protocol §8.3.1); after unlock the process samples ~1-in-10
-- (Method B), and those post-unlock samples are NOT baseline evidence. Without
-- this bound they would mask a later deletion/regression of the original baseline
-- (post-unlock samples keep the lifetime count ≥30 even after the pre-unlock
-- population drops below it), defeating the invariant. `sampling_time` is NOT
-- NULL (schema), so the only NULL in the predicate is `method_b_unlocked_at`
-- itself: a row claiming Method B without an unlock timestamp counts 0 samples
...
-- save" stance: deleting a pre-unlock baseline sample regresses an already-
-- Method-B process's eligibility, so an unrelated edit must re-assert the
-- invariant rather than silently preserve a now-invalid configuration.
--
...
BEGIN
  IF NEW.sampling_method = 'method_b' THEN
    -- Only the PRE-UNLOCK baseline counts (samples before method_b_unlocked_at);
    -- post-unlock Method-B samples are not baseline evidence and must not mask a
    -- later regression. A null unlock timestamp yields 0 (fail closed).
    SELECT count(*)
