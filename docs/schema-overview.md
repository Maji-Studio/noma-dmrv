...
- **`certifier_biochar_applications` journals Biochar Application creation at the Removal-versioned allocation-slice grain.** One row represents one `(Application, credit batch/Production Batch, Removal submission version)` slice and retains the linked submission, Production Batch and Storage Location registrations, exact submitted payload/hash, external identity, nullable observed GHG Entry/Removal identity, and ordinary `creating` or `confirmed` lifecycle state. The provider create contract cannot set a GHG Entry association, so a matching remote Application may be confirmed while both observed association fields remain null; any non-null association must match the current GHG Entry. Supersession owns a new supplier reference and remote Biochar Application; an exact confirmed retry reads that version's persisted external identity and performs zero writes. The provider arrival mass is the slice's immutable allocated wet kg and departure is zero; commingled slices therefore partition the physical Application total. An unconfirmed claim reconciles every bounded list page by its exact stable supplier reference before POST. Payload or non-null current-GHG identity drift fails closed.
