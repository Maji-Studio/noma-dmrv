...
| `customer_locations` | Parties | Normalized delivery/application locations per customer with structured address fields (country, state/region, city), per-location facility distance, and a default-destination flag (one default per customer, partial-unique enforced). | Multi-field customers, geospatial destination accuracy. | `src/db/schema/parties.ts` |
