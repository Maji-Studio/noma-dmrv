...
| `facilities` | Facilities | Master record for production sites, including required facility timezone (`UTC` default) and the authoritative durability tier (`durability_option`, ADR 0021) inherited by the facility's batches, samples, and removal template. | Facility onboarding, local-time reporting, durability-tier declaration. | `src/db/schema/facilities.ts:9` |
...
| `production_run_readings` | Production | Time-series telemetry (temperature, pressure, dryer/reactor drive frequency) imported from a canonical UTC-timestamped CSV. | Monitoring-plan evidence, compliance checks, diagnostics. | `src/db/schema/production.ts:118` |
...
| `production_processes` | Production | Sampling-regime campaign keyed `(facility, feedstock)`, spanning reactors (Biochar Protocol §8.3.1; ADR 0016). Owns `sampling_method` (moved off `reactors`), `established_at` baseline epoch, and the Method-B unlock columns `method_b_unlocked_at` + captured prerequisites (`agreed_baseline_size`, `random_sampling_plan_ref`, `moisture_pathway`; ADR 0017 Track 2), backstopped by the `enforce_process_method_b_minimum_samples` trigger (migration `0060`). Non-unique lookup index — sequential processes per pair over time; find-or-created on credit-batch create. | Method A/B sampling scope, baseline counting, credit-batch process linkage. | `src/db/schema/production-processes.ts` |
| `credit_batches` | Credits | The protocol production batch (ADR 0016): one feedstock, facility-scoped, ≤ 1 month under Isometric. Carries derived `feedstock_type_id` (NOT NULL) + `production_process_id`. Stores no aggregate totals (issue #285): applied weight, CO2e stored, and the feedstock masses behind the >25% ineligible-biomass cap (P0-01) are derived on read from member applications / run-feedstock lineage; project emissions and counterfactual are registry-owned (ADR 0018). DB `check` enforces the ≤ 1-month Isometric window. Durability tier is inherited (join-derived) from the facility — no per-batch `durability_option` column (dropped in `0069`, ADR 0021). | Registry submission prep, per-credit-batch lab sampling, derived ineligible biomass fraction reporting. | `src/db/schema/credits.ts:22` |
