...
| Production processes (sampling campaigns) | `src/db/schema/production-processes.ts` |
...
- **Some rules live only in raw SQL**, not in Drizzle: e.g. the Method-B ≥30 sample-floor trigger `enforce_process_method_b_minimum_samples` (`drizzle/0060_process_method_b_minimum_samples_guard.sql`), alongside the Method-B prerequisite `check` in `production-processes.ts`. Reading the TS schema alone under-reports what the DB rejects.
...
- **`production_processes` owns `sampling_method`**, keyed `(facility, feedstock)` — it is *not* on `reactors`. See [ADR 0016](./adr/0016-credit-batch-is-production-batch-production-process-scopes-sampling.md), [ADR 0017](./adr/0017-method-b-unlock-registry-computes-noma-gates-and-previews.md).
