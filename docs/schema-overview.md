...
| `certifier_sources` | Certification | Normalizes external source references by provider/type. | Stable mapping of provider-specific IDs used in submissions. | `src/db/schema/certification.ts:64` |
| `certification_submissions` | Certification | Immutable versioned submission history with payload snapshots. A GHG Statement is one row keyed `localEntityType:'creditBatch'`; a Removal is one row keyed `localEntityType:'productionRun'`. | Submission lifecycle tracking, auditability, resubmission/versioning. | `src/db/schema/certification.ts:86` |
