...
| `credit_batches` | Credits | The protocol production batch (ADR 0016): one feedstock, facility-scoped, ≤ 1 month under Isometric. Carries derived `feedstock_type_id` (NOT NULL) + `production_process_id`. Stores no aggregate totals (issue #285): applied weight, CO2e stored, and the feedstock masses behind the >25% ineligible-biomass cap (P0-01) are derived on read from member applications / run-feedstock lineage; project emissions and counterfactual are registry-owned (ADR 0018). DB `check` enforces the ≤ 1-month Isometric window. | Durability pathway selection (locked after `verified`/`issued`), registry submission prep, per-credit-batch lab sampling, derived ineligible biomass fraction reporting. | `src/db/schema/credits.ts:22` |
