...
- **`deliveries.truck_mass_on_arrival_kg` and `truck_mass_on_departure_kg` are optional observed gross/tare evidence in the shared `massKg` family.** When both exist, arrival must be at least departure. They remain optional for ordinary delivery operations. As a temporary sandbox-only submission fallback, an absent arrival uses a positive `delivered_wet_mass_kg` and an absent departure uses `0 kg`; explicit observations remain authoritative and the fallback is not persisted as observed evidence.
