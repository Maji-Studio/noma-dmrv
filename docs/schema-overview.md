...
| Gated registry Biochar Application identities | `src/db/schema/certifier-biochar-applications.ts` |
...
- **`certifier_biochar_applications` reserves the registry identity at the allocation-slice grain.** One row represents one `(Application, credit batch/Production Batch)` slice and retains the linked Production Batch and Storage Location registrations, submitted payload/hash, observed GHG Entry identity, and correction lifecycle. Rows remain gated until Isometric defines the required mass/rate/allocation contract; the current code has no Biochar Application POST path.
