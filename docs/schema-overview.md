...
- **`certifier_biochar_applications` journals Biochar Application creation at the allocation-slice grain.** One row represents one `(Application, credit batch/Production Batch)` slice and retains the linked Production Batch and Storage Location registrations, exact submitted payload/hash, external identity, observed GHG Entry/Removal identity, and lifecycle/correction/gate state. A durable `gated` claim is written before POST; retries reconcile every bounded list page by the exact stable supplier reference and confirm the existing remote record instead of duplicating it. Payload or identity drift fails closed. Sandbox Removal submission is the only automatic call path; production remains blocked.
- **`deliveries.truck_mass_on_arrival_kg` and `truck_mass_on_departure_kg` are optional observed gross/tare evidence in the shared `massKg` family.** When both exist, arrival must be at least departure. They remain optional for ordinary delivery operations, but sandbox Biochar Application submission requires both observations and never substitutes delivered wet mass or zero as evidence.
