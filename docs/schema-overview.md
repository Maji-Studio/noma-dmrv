...
- **`certifier_storage_locations` journals one reusable agricultural site per Customer Location.** The row fixes the Isometric `slc_...` identity, project snapshot, stable supplier reference, submitted payload/hash, and operator-visible drift state. It is distinct from facility inventory-bin `storage_locations`; application create/update never writes it.
