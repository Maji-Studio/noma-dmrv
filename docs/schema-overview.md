...
- **`certifier_biochar_applications` journals Biochar Application creation at the allocation-slice grain.** One row represents one `(Application, credit batch/Production Batch)` slice and retains the linked Production Batch and Storage Location registrations, exact submitted payload/hash, external identity, observed GHG Entry/Removal identity, and ordinary `creating` or `confirmed` lifecycle state. The provider arrival mass is the slice's immutable allocated wet kg and departure is zero; commingled slices therefore partition the physical Application total. A claim is written before POST in every configured Isometric environment. Retries reconcile every bounded list page by the exact stable supplier reference and confirm the existing remote record instead of duplicating it. Payload or identity drift fails closed.
