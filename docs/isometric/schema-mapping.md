...
| 1,000-year durability submission | `src/fn/certification/durability-measurement-samples.ts`, `src/lib/isometric/transformers/sequestration-binding.ts`, current preview/evidence builders, and tests | code-implemented, externally pending | Current sampled contract binds paired total carbon, directly measured inorganic carbon, `s_fraction`, and product mass to `biochar_sequestration_1000_year_f_durable_max`; local explanation uses per-replicate organic carbon and the raw/0.95-capped durability. Registry authority is preserved. Production is blocked; sandbox template migration and Isometric confirmation remain outstanding |
