...
| `sampling.process_method_b_minimum_samples` | `production_processes` | `sampling_method = method_b` | minimum 30 prior samples in the process | Deferred to ADR 0017 with the Method-B unlock. Current Phase 1 stores the process sampling regime and keeps DEC on Method A. |
| `sampling.credit_batch_method_b_cadence` | `credit_batches` | process `sampling_method = method_b` | sampled credit batches in period must satisfy `>= 1/10` cadence | Deferred to ADR 0017. The durability submission gates are credit-batch-grained as of Tier-1, but the Method-B 1-in-10 cadence engine specifically remains reactor-grained until ADR 0017 re-keys it to the process. Inert today — DEC runs Method A everywhere. |
