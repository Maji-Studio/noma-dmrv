...
| `sampling.credit_batch_method_b_cadence` | `credit_batches` | linked `reactor.sampling_method = method_b` | sampled production runs in period must satisfy `>= 1/10` cadence | Planned (not implemented in current migration) |
