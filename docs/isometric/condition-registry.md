...
| `application.evidence_method` | `applications` + `documents` | `evidence_method = visual` or `boundary` | visual: uploaded application photos for `metadata.evidenceRole in (stockpile, spreading, incorporation)` with `metadata.geotagStatus = present`; boundary: non-null `gis_boundary` plus typed logbook evidence (`metadata.logbookEvidenceType in (weighbridge, inventory, affidavit)` or semantic document type) | Certification readiness check (`buildApplicationEvidenceGaps`) + dashboard evidence health. An uploaded GeoJSON file is retained as a `gis_boundary` application document; a boundary entered through the paste path retains no source file, so retention is not guaranteed for every boundary. Upload accepts photos missing EXIF and records the gap in metadata rather than rejecting. |
