...
| `sampling.credit_batch_method_b_cadence` | `credit_batches` | process `sampling_method = method_b` | sampled credit batches in period must satisfy `>= 1/10` cadence | Deferred to ADR 0017. Current submission gates remain fail-closed at legacy run/reactor grain until the cadence engine is re-keyed to process/credit-batch grain. |
