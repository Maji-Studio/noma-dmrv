...
| **5** — Time-series + bulk | ⚪ Legacy prototype retained; operator workflow deferred | The 2026-05-29 prototype can build a `DataUploadSubmission` from clock-aligned `production_run_readings`, with `certifier_sensors`, facility mapping, journaled idempotency, and the validated Parquet writer retained in code. It is not a live operator workflow: production-run readings are now stored as unchanged `sensor_data` CSV documents, the upload UI does not populate row-level readings, and the telemetry panel has no mounted operator entry point. Decide the required Isometric evidence path before reviving or removing the prototype; tracked in `docs/open-questions.md`. **Slice B** (`POST /biochar_applications`) and **Slice C** (`MonitoringSubmission`) remain deferred. |
...
src/fn/certification/                  # withAction-wrapped server actions
  ...
  ├─ submit-telemetry.ts               # legacy ADR 0006 prototype; no operator entry point
  ├─ shared.ts
  └─ index.ts
