...
| Method regime is retroactive | P0 | Protocol integrity / Product decision | Create historical Method-A batches, unlock their process, then re-evaluate an earlier batch. | The sampling obligation that applied when a batch began should remain auditable. Actual: readiness reads the process's current method, so unlock may reinterpret historical batches as Method B. | Method is stored only on `production_processes` and read live for member batches; no per-batch regime snapshot/effective interval. | Decision resolved 2026-07-12: the regime is fixed at credit-batch production-period start; Method B applies only to batches starting after `method_b_unlocked_at`. Implement this authoritative as-of rule in a dedicated follow-up. | Decision documented in CONTEXT.md and ADR 0017; implementation remains open. |
| Method-B baseline accepts non-representative rows | P0 | Protocol integrity | Add 30 complete samples clustered on one day and dated before the process/run, then unlock. | Baseline samples should fall within the process and represent distributed independent sampling. Actual: rows warn as clustered but count 30/30 and unlock. | Eligibility is principally a row count with an upper `asOfDate`; it lacks an `establishedAt` lower bound and hard independence predicate. | Lower-bound decision resolved 2026-07-12: `establishedAt` is the operator-entered real operational start, not row creation, and earlier samples never count. Implement that bound separately. Independent-sampling semantics still require protocol agreement. | Reproduced in the prior run; lower-bound decision documented, enforcement open. |
...
| Method-B “lock” semantics | P1 | The database does not make all baseline evidence immutable. It preserves a count floor of 30: eligible chemistry edits and surplus-sample deletion can still succeed. Calling this a fully locked baseline was misleading. | Decision resolved 2026-07-12: the ≥30 floor remains, but unlock alone does not freeze ordinary corrections. Once any submitted Removal/GHG Entry depends on a sample, direct edits are blocked even if its own batch remains draft. A public `updateSample` regression now proves qualification-breaking redating rolls back while ordinary unsubmitted corrections persist. Dependency snapshot/enforcement remains follow-up work related to #200/#391; no migration was added. |
...
| Missing credit-batch ID | P2 | `/credit-batches/__missing__` reaches an error boundary/500 rather than a designed not-found result. | Fixed narrowly: malformed UUIDs short-circuit before DB access; malformed, absent, and foreign-organization IDs render the same designed 404 with a back link. Unit, E2E, and screenshot verification passed. |
| Missing supplier/customer IDs | P2 | Missing detail IDs retry for roughly seven seconds and then show a generic error instead of a proper not-found state. | Fixed without merging the stale detail-route branch: server UUID/org-scoped preflights prevent client retries, and designed 404s cover malformed, absent, and foreign-organization IDs. Unit, E2E, and screenshot verification passed. |
| Empty-state duplication | P3 | Applications and Production Processes can repeat the select-facility empty-state wording. | Fixed at the page-level facility gate; each page now renders one shared prompt and no inactive table/create/unlock controls. Component tests and browser screenshots passed. |
| Dev-mode 404 instrumentation noise | P3 | Rapid intentional 404 navigation can log a Next development `Performance.measure` negative-timestamp error naming the detail page even though the response and designed 404 UI are correct. | Recorded as development-framework instrumentation noise; it caused no assertion/UI failure and was not used to weaken the 404 checks. Recheck after a Next upgrade or if it appears in a production build. |

PR #432 is merged; the refreshed GitHub survey found no other open PR that
needed merging into this follow-up. Existing
...
### Bounded edge-case fix verification

- Reset the local database through migrations; no Isometric credentials were
  configured and no registry mutation was attempted.
- Focused Vitest: 4 files / 19 tests passed, including route preflights, both
  facility gates, and the public Method-B sample-update behaviors.
- Focused Chromium: 5/5 tests passed. The suite covers malformed and absent IDs
  for all three detail routes, foreign-organization IDs for all three, working
  back links, and the Applications/Production Processes facility gates.
- Five browser screenshots were captured and visually inspected: three designed
  404 screens plus the two single facility prompts. No generic error, retry
  delay, table, create action, unlock action, or start-process control appeared.
- `pnpm typecheck` passed. `pnpm lint` passed with zero errors and the same 21
  pre-existing warnings.
- The Codex CLI computer-use wrapper recursively invoked itself and was stopped;
  the skill's Playwright fallback supplied the completed browser verification.
  No user login intervention was required.
