# 2026-07-26 UI-only operational stress test
...
test. Source-derived expectations are not execution evidence. Only behavior
visibly observed through the bundled `@oai/sky` Computer Use runtime may be
recorded as a product pass, failure, or defect.
...
| --- | --- | --- | --- |
| 1 | 2026-07-26 04:01 | `mcp__node_repl__js` → plugin-owned `@oai/sky` → Chrome | Blocked: Mac locked; manual unlock required |
| 2 | 2026-07-26 04:07 | `mcp__node_repl__js` → plugin-owned `@oai/sky` → Chrome | Blocked: identical lock condition |
| 3 | 2026-07-26 04:08 | `mcp__node_repl__js` → plugin-owned `@oai/sky` → Chrome | Blocked: identical lock condition; formal threshold met |
...
None observed because Chrome and staging have not become visible. No alternative
...
authorized visible Computer Use runtime cannot acquire Chrome state while macOS
is locked. The safety boundary forbids substituting browser scripting, direct
APIs, GitHub CLI, database access, request manipulation, or session
manipulation.
