...
2. The local database reset is explicitly authorized for this run. Run:
   pnpm db:reset
   Never reset staging or production.
3. Use pnpm only.
...
directory for `<ARTIFACT_DIR>`, write the prompt to that directory, and run Codex with
