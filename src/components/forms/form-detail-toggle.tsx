...
export function FormDetailToggle({ value, onChange }: {
  ...
  const name = useId();
  return <fieldset data-presentation-control className="inline-flex gap-2 bg-[var(--color-surface-light)] p-2">
    ...
    {OPTIONS.map((option) => <label key={option.value} className="relative cursor-pointer">
      ...
      <span className={cn(
        "flex min-h-32 items-center px-10 body-caption peer-focus-visible:outline-2 peer-focus-visible:outline-[var(--color-interaction)]",
