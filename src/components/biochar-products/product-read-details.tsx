...
export function savedProductComposition(product: BiocharProductWithRelations) {
  ...
  return { ingredients, sourceWetKg, sourceDryKg, sourceTotalKg, productTotalKg, sourceComponents, productComponents: productCompositionComponents(sourceComponents, ingredients.flatMap(ingredient => ingredientComponents(ingredient, true))) };
}
  ...
  return <div className="space-y-20">
    ...
    <DetailSpine numbered sections={[
      { title: "Placement", fields: [{ label: "Mixing and placement date", value: formatDate(product.placedAt) }] },
      {
        ...
        content: <div className="space-y-12"><ZeroSourceBiocharWarning sourceBiocharMassKg={composition.sourceWetKg} />
          {detailed && <CompositionCard title="Source composition" totalKg={composition.sourceTotalKg} components={composition.sourceComponents} details={<>
            <CalculationFacts facts={[{ label: "Biochar (wet)", massKg: composition.sourceWetKg }, { label: "Dry biochar", massKg: composition.sourceDryKg }, { label: "Added water", massKg: product.waterAddedKg }]} />
            <p className="body-small">{product.sourceAllocatedDryMassKg !== null ? "Dry biochar uses the recorded source allocation. Later moisture edits do not change it." : "Dry biochar is derived from the recorded source wet mass and moisture."}</p>
            {sourceBinId && <div className="space-y-8"><h4 className="body-small font-medium">{sourceBinName}</h4><OutputStockHistory quietTrigger storageLocationId={sourceBinId} facilityId={product.facilityId} triggerLabel="View source lots" /></div>}
          </>} />}
        </div>,
      },
      {
        ...
        content: detailed && <div className="space-y-16">{composition.ingredients.map(ingredient => <CompositionCard key={ingredient.formulationIngredientId} title={`${ingredient.feedstockTypeName} composition`} totalKg={ingredient.massKg ?? null} components={ingredientComponents(ingredient, true)} details={<>
          ...
          <p className="body-small">Dry solids use the saved ingredient snapshot, not current bin moisture.</p>
        </>} />)}</div>,
      },
      {
        title: "Product",
        ...
        content: detailed && <CompositionCard title="Product composition" totalKg={composition.productTotalKg} components={composition.productComponents} details={<>
          <CalculationFacts facts={[{ label: "Source biochar (wet)", massKg: composition.sourceWetKg }, ...composition.ingredients.map(ingredient => ({ label: `${ingredient.feedstockTypeName} (wet)`, massKg: ingredient.massKg ?? null })), { label: "Added water", massKg: product.waterAddedKg }, { label: "Wet total", massKg: composition.productTotalKg }]} /><p className="body-small">{product.sourceAllocatedDryMassKg !== null ? "Dry biochar uses the recorded source allocation. Later moisture edits do not change it." : "Dry biochar is derived from the recorded source wet mass and moisture."} Ingredient dry solids come from the saved snapshot, not current bin moisture.</p>
          {product.storageLocation && <OutputStockHistory quietTrigger storageLocationId={product.storageLocation.id} facilityId={product.facilityId} triggerLabel="View stock history" />}
        </>} />,
      },
      ...(detailed ? [{ title: "Derived transport", fields: [], content: <TransportLegsSummary entityType="biochar" entityId={product.id} emptyMessage="Transport legs are derived from this product's deliveries. Record a delivery to a destination with a distance from the facility." /> }] : []),
    ]} />
  </div>;
}
