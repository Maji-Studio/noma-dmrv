...
function Harness({ frozen = false, saved = false }: { frozen?: boolean; saved?: boolean }) {
  const form = useForm({ defaultValues: { ingredientBins: [{ storageLocationId: "bin", massKg: 100, moistureContentPercent: saved ? 12 : null, moistureSource: saved ? "operator_override" : "weighted_remaining" }] } });
...
describe("IngredientMoistureField", () => {
  it("prefills the weighted remaining stock and records an operator override when edited", async () => {
    let renderer!: ReactTestRenderer;
    await act(async () => { renderer = create(<Harness />); });
    expect(renderer.root.findByType("input").props.value).toBe(100 / 15);
