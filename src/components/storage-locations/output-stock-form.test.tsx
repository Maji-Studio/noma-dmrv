...
vi.mock("./output-stock-preview", () => ({ OutputStockPreview: () => <div>Preview</div>, OutputStockAllocations: () => <div>Allocations</div> }));
...
describe("OutputStockForm", () => {
  it("allows zero count without moisture and retries a correction with the same key and current basis", async () => {
    ...
    const submit = () => renderer.root.findByType("form").props.onSubmit({ preventDefault: vi.fn(), persist: vi.fn() });
