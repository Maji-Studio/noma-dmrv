...
export function OutputStockHistory({ storageLocationId, facilityId }: { storageLocationId: string; facilityId: string }) {
    ...
    <Modal isOpen={open} onClose={() => { setOpen(false); setOriginal(undefined); }} ariaLabel="Stock history" width="lg">
      <div className="space-y-20">
        ...
        {original ? <OutputStockForm storageLocationId={storageLocationId} facilityId={facilityId} kind={originalKind(original) === "count" ? "count" : originalKind(original) === "delivery" ? "delivery" : "loss"} original={original} onCancel={() => setOriginal(undefined)} onRecorded={() => setOriginal(undefined)} /> : <>
          ...
          {history.data?.toReversed().map(entry => <article key={entry.id} className="space-y-8 border-t border-[var(--color-border-tertiary)] pt-16">
            <h4 className="body-small font-semibold">{entry.kind}{entry.correctsMovementId ? `, corrects ${history.data?.find(original => original.id === entry.correctsMovementId)?.reason ?? "a previous entry"}` : ", original entry"}</h4>
