 ...
 * method, its Method-B baseline progress (N / 30 eligible replicate samples),
 * and its cadence status. It is read-only today; the Method-B unlock action and
 * the protocol-cited explanation surface ship with Track 2.
 */
"use client";
...
function createColumns(): ColumnDef<ProductionProcessSummary>[] {
  return [
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        // processes show their progress toward the ≥30-sample bar.
      ...
      cell: ({ row }) => {
        ...
        const detail =
          p.samplingMethod === "method_b"
            ? `${p.sampledBatches}/${p.requiredSampledBatches} batches (≥1 per 10)`
    ...
    <div className="container-max page-shell">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-24">
        ...
        <StatCard
          ...
          description="Cleared the ≥30-sample baseline; can unlock Method B"
          isLoading={isLoading}
        />
        <StatCard
          ...
          description="Processes sampling ≥1 per 10 batches"
          isLoading={isLoading}
        />
      </div>
