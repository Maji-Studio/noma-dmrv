 ...
 *
 * Design tokens and the shared document chrome (masthead, claim frame, table
 * frame, apparatus, footer) come from ./pdf-theme — the single source both
 * evidence ledgers draw on; only transport-specific styles live here.
 */
...
// footer) comes from the shared theme; the entries below are transport-specific.
const styles = {
  ...theme,
  ...StyleSheet.create({
  // Claim band body
  ...
  // Apparatus legend keys are short ("Map · derived"): 72pt column.
  legendKey: { ...theme.legendKey, width: 72 },
  }),
};
