...
export function getCompositionIngredientDraws(
  ...
  for (const ref of getCompositionIngredientRefs(composition)) {
    ...
    // A source bin is optional. Its wet mass still belongs to the blend, but
    // only explicitly tracked bins produce a feedstock inventory draw.
...
export async function validateCompositionIngredientBins(
  ...
  if (ingredientRefs.length > 0) {
    ...
    if (ingredientIds.length !== ingredientRefs.length) {
      throw new SafeError(
