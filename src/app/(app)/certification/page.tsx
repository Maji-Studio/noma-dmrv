/**
 * Certification — landing page
 * Tile grid linking to each certification surface (Removals, GHG Statements).
 * Renders inside the (app) shell; access is gated by the (app) layout's
 * requireAuth() guard. Provider-neutral by design — Verra / Gold Standard /
 * CSI surfaces may be added later; today every tile is Isometric-specific.
 */
import type { CSSProperties, ElementType } from "react";
import {
  ArrowRight,
  ClipboardText,
  SealCheck,
} from "@phosphor-icons/react/dist/ssr";
import { ProjectEmissionsDriftPanel } from "@/components/certification/project-emissions-drift-panel";
import { Card } from "@/components/ui/card";

interface CertificationTile {
  href: string;
  icon: ElementType;
  title: string;
  description: string;
  /** Accent color token — drives the tile's icon, edge bar, and hover states. */
  accent: string;
}

const CERTIFICATION_TILES: CertificationTile[] = [
  {
    href: "/certification/removals",
    icon: SealCheck,
    title: "Removals",
    description:
      "Group credit batches into Isometric Removals and submit each one for verification.",
    accent: "var(--clr-pink)",
  },
  {
    href: "/certification/ghg-statements",
    icon: ClipboardText,
    title: "GHG Statements",
    description:
      "Create period-anchored GHG Statements that roll up multiple Removals, then submit them to the verifier.",
    accent: "var(--clr-purple)",
  },
];

export default function CertificationHomePage() {
  return (
    <div className="container-max py-32 flex flex-col gap-32">
      <header className="flex flex-col gap-8">
        <h1 className="title-heading-2">Certification</h1>
        <p className="body-medium text-[var(--color-text-secondary)] max-w-[560px]">
          Submit MRV data to the carbon registry for verification. Choose a
          surface to work with.
        </p>
      </header>

      <ProjectEmissionsDriftPanel />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-24">
        {CERTIFICATION_TILES.map((tile) => {
          const Icon = tile.icon;
          return (
            <Card.Root
              key={tile.href}
              padding="none"
              radius="none"
              style={{ "--tile-accent": tile.accent } as CSSProperties}
            >
              <Card.Link href={tile.href} ariaLabel={tile.title}>
                <div className="relative flex h-full flex-col gap-16 p-24">
                  {/* Edge accent bar — echoes the sidebar's active indicator */}
                  <span
                    aria-hidden
                    className="absolute inset-y-0 left-0 w-[2px] bg-[var(--tile-accent)] opacity-40 transition-opacity duration-200 group-hover:opacity-100"
                  />

                  {/* Icon */}
                  <span
                    aria-hidden
                    className="flex size-44 items-center justify-center border border-[var(--color-border-tertiary)] text-[var(--tile-accent)] transition-colors duration-200 group-hover:border-[var(--tile-accent)]"
                  >
                    <Icon size={22} weight="duotone" />
                  </span>

                  {/* Title + description */}
                  <div className="flex flex-col gap-8">
                    <h2 className="title-heading-3">{tile.title}</h2>
                    <p className="body-medium text-[var(--color-text-secondary)]">
                      {tile.description}
                    </p>
                  </div>

                  {/* Footer affordance */}
                  <div className="mt-auto flex items-center gap-8 pt-16">
                    <span className="title-chapter-title text-[var(--color-text-tertiary)] transition-colors duration-200 group-hover:text-[var(--color-text-primary)]">
                      Open
                    </span>
                    <ArrowRight
                      aria-hidden
                      size={14}
                      weight="bold"
                      className="text-[var(--tile-accent)] transition-transform duration-200 group-hover:translate-x-[3px]"
                    />
                  </div>
                </div>
              </Card.Link>
            </Card.Root>
          );
        })}
      </div>
    </div>
  );
}
