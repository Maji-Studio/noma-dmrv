...
describe("order cross-parent guards", () => {
  ...
  beforeAll(async () => {
    const fixture = await db.transaction(async (tx) => {
      const [facility] = await tx
        .insert(facilities)
        .values({ code: `FAC-OCG-${tag}`, name: `OCG Facility ${tag}` })
        .returning({ id: facilities.id });
      const [product] = await tx
        .insert(biocharProducts)
        .values({ code: `BP-OCG-${tag}`, facilityId: facility.id })
        .returning({ id: biocharProducts.id });
      const [customerA] = await tx
        .insert(customers)
        .values({ code: `CU-OCG-A-${tag}`, name: `OCG Customer A ${tag}` })
        .returning({ id: customers.id });
      const [customerB] = await tx
        .insert(customers)
        .values({ code: `CU-OCG-B-${tag}`, name: `OCG Customer B ${tag}` })
        .returning({ id: customers.id });
  ...
  it("rejects changing an order customer while preserving another customer's location", async () => {
    await expect(
      updateOrder(TEST_USER_ID, orderId, { customerId: customerBId }),
    ).rejects.toThrow("Delivery location belongs to a different customer");
  });

  it("rejects changing an order location to another customer's location", async () => {
    await expect(
      updateOrder(TEST_USER_ID, orderId, { customerLocationId: locationBId }),
    ).rejects.toThrow("Delivery location belongs to a different customer");
  });
});
