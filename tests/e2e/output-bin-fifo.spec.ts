...
test.describe("Output-bin conserved FIFO", () => {
  ...
  test("creates a Pure formulation product with an actual placement date and treats drying as no loss", async ({ adminPage: page, testUsers }, info) => {
    ...
    await expect(page.getByText(/0 kg dry biochar removed/)).toBeHidden();
    await page.getByRole("radio", { name: "Detailed", exact: true }).locator("..").click();
    await expect(page.getByText(/0 kg dry biochar removed/)).toBeVisible();
  ...
  test("blocks a dry shortage, saves a spanning delivery, and applies both batch shares", async ({ adminPage: page, testUsers }, info) => {
    ...
    await expect(preview.getByText(/1,150 kg dry biochar removed/)).toBeHidden();
    await expect(preview.getByRole("button", { name: "Stock history", exact: true })).toHaveCount(0);
    await page.getByRole("radio", { name: "Detailed", exact: true }).locator("..").click();
    await expect(preview.getByText(/1,150 kg dry biochar removed/)).toBeVisible();
    await expect(preview.getByText("Before loading", { exact: true })).toBeHidden();
    await preview.getByRole("button", { name: /^Show details for/ }).click();
    await expect(preview.locator("dl > div").filter({ hasText: "Before loading" })).toContainText("2,600 kg wet estimate");
    await expect(preview.locator("dl > div").filter({ hasText: "After loading" })).toContainText("600 kg wet estimate");
    ...
    await preview.getByText("After loading", { exact: true }).scrollIntoViewIfNeeded();
  ...
  test("reconciles without loss, records and corrects loss, and closes zero without moisture", async ({ adminPage: page, testUsers }, info) => {
    ...
    await expect(page.getByRole("radio", { name: "Simple", exact: true })).toBeChecked();
    await expect(page.getByText(/70 kg dry biochar removed/)).toBeHidden();
    await page.getByRole("radio", { name: "Detailed", exact: true }).locator("..").click();
    await expect(page.getByText(/70 kg dry biochar removed/)).toBeVisible();
    ...
    await expect(history.getByText(/7 kg dry biochar removed/)).toBeHidden();
    await history.getByRole("radio", { name: "Detailed", exact: true }).locator("..").click();
    await expect(history.getByText(/7 kg dry biochar removed/)).toBeVisible();
    ...
    await expect(page.getByRole("radio", { name: "Simple", exact: true })).toBeChecked();
    await expect(page.getByText(/343 kg dry biochar removed/)).toBeHidden();
    await page.getByRole("radio", { name: "Detailed", exact: true }).locator("..").click();
    await expect(page.getByText(/343 kg dry biochar removed/)).toBeVisible();
    await page.getByRole("button", { name: "Reconcile stock", exact: true }).last().click();
    await expect(page.getByRole("dialog")).toHaveCount(0);
